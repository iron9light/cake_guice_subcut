...
private[inject] class InjectableInstanceProvidor[I <: Any](fn: (BindingModule => I)) {
  def instance(bindingMolude: BindingModule) = fn(bindingMolude)

  override def toString = "InjectableInstanceProvider[%s]".format(fn.toString)
}